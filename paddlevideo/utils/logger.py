...
def setup_logger(output=None, name="paddlevideo", level="INFO"):
    ...
    if output is not None:
        if output.endswith(".txt") or output.endswith(".log"):
            ...
            # output = "log/paddlevideo_my_log/"
            # filename = os.path.join(output, ".log.txt")
            # filename = os.path.join(output, "log_modify_ucf101_all.txt")
            # filename = os.path.join(output, "log_modify_my_retrain_retrain.txt")
            # filename = os.path.join(output, "log_modify_my_ceshi.txt")        # 有ucf101 modify
            filename = os.path.join(output, "log_ucf101_all.txt")
            # filename = os.path.join(output, "log_modify_for_visul——new.txt")
...
def get_logger(name, output=None):
    # print("get_logger中output：", output)
    # print("get_logger中name：", name)
    logger = logging.getLogger(name)

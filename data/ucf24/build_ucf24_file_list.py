...
def parse_directory(path,
    ...
    if level == 1:
        ...
        frame_folders = glob.glob(os.path.join(path, '*', '*'))
        # print("frame_folders:", frame_folders)    # frame_folders: ['/mnt/sdb1/swf/data/UCF24/rgb-images/SoccerJuggling/v_SoccerJuggling_g09_c01', '/mnt/sdb1/swf/data/UCF24/rgb-images/SoccerJuggling/v_SoccerJuggling_g23_c03',
    else:
...
def parse_args():
    ...
    parser.add_argument(
        '--frame_path', type=str, default='/mnt/sdb1/swf/data/UCF24/rgb-images/', help='root directory for the frames')
...
def main():
    ...
    # print("len(split_tp):", len(split_tp))
    # print("args.num_split:", args.num_split)
    ...
    for i, split in enumerate(split_tp):
        ...
        filename = 'ucf24_train_split_{}_{}.txt'.format(i + 1, args.format)
        with open(os.path.join(out_path, filename), 'w') as f:
            ...
            #     cnt = len(lst)
            #     return cnt
            # print("count_files(PATH):", count_files(PATH))

            # print("[os.path.join(PATH, item) for item in lists[0]]:", [os.path.join(PATH, item) for item in lists[0]])

            f.writelines([os.path.join(PATH, item) for item in lists[0]])


        filename = 'ucf24_val_split_{}_{}.txt'.format(i + 1, args.format)
        with open(os.path.join(out_path, filename), 'w') as f:

...
@HEADS.register()
class ppTSMHeadImprove(TSNHead):
    """ ppTSM Head
    Args:
    ...
    def __init__(
        ...
        # # todo: 应用C2f
        # self.c2f = C2f(c1=in_channels, c2=in_channels)
        ...
        # # todo: 加MultiDilatelocalAttention
        # print("data_format:", data_format)
        # print("input_channel:", in_channels)
        self.MultiDilatelocalAttention = MultiDilatelocalAttention(dim=in_channels, num_heads=8, dilation=[1, 2])
    ...
    def forward(self, x, num_seg=None):
        ...
        """
        # print("ppTSMHeadImprove -- forward")
        ...
        # #todo: 应用CBAM模块
        # x1 = self.cbam(x)
        # 应用scconv空间和通道重建卷积     # (top1 acc)0.65
        # x = self.scconv(x)

        # # todo: 用MultiDilatelocalAttention(dim=16, num_heads=8, dilation=[1, 2])      (top1 acc)0.95
        # print("x.shape-before:", x.shape)
        x = self.MultiDilatelocalAttention(x).transpose([0, 3, 1, 2])
        # print("x.shape-after:", x.shape)

        # todo: 双分支注意力网络cbam + MultiDilatelocalAttention，最后将这双分支融合（+）起来 【 + 】    (top1 acc)0.9
        # x = x1 + x

        # todo: 双分支注意力网络cbam + MultiDilatelocalAttention。【 concat 】在特定维度上拼接CBAM和MultiDilateLocalAttention的输出，比如通道维度(axis=1)
        # x = paddle.concat([x1, x], axis=1)

        # todo: 双分支注意力网络cbam + MultiDilatelocalAttention，最后将这双分支融合起来  【*】   (top1 acc)0.95
        # x = x1 * x

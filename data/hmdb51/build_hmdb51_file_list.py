...
def parse_ucf101_splits(level):
    ...
    for i in range(1, 2):
        ...
        test_list = [
            line2rec(x)
            for x in open('./annotations/vallist{:02d}.txt'.format(i))
        ]
...
def parse_args():
    ...
    parser.add_argument(
        '--frame_path', type=str, default='/mnt/sdb1/swf/data/hmdb51/hmdb51_frames/', help='root directory for the frames')
...
def main():
    ...
    for i, split in enumerate(split_tp):
        ...
        filename = 'hmdb51_train_split_{}_{}.txt'.format(i + 1, args.format)
        ...
        filename = 'hmdb51_val_split_{}_{}.txt'.format(i + 1, args.format)
        with open(os.path.join(out_path, filename), 'w') as f:

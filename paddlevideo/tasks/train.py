...
def train_model(cfg,
    ...
    # logger = get_logger("log/paddlevideo_my_log/modify_ucf24.log", "log/paddlevideo_my_log/modify_ucf24.log")
    batch_size = cfg.DATASET.get('batch_size', 8)
    valid_batch_size = cfg.DATASET.get('valid_batch_size', batch_size)
    writer = LogWriter(logdir="/mnt/sdb1/swf/project/PaddleVideo/log/scalar_test/aaa")
    ...
    if parallel:
        print("paddle.DataParallel(model)")
    ...
    for epoch in range(0, cfg.epochs):
        ...
        print("aa:", aa)
        # 获取损失值的平均值
        loss_meter = record_list['loss']  # 这里假设 'loss' 是 metrics 字典中的一个键
        loss_avg = loss_meter.avg  # 假设 AverageMeter 对象有一个 'avg' 属性
        ...
        top1 = record_list['top1']
        top1_avg = top1.avg
        print("top1_avg:", top1_avg)

        top5 = record_list['top5']
        top5_avg = top5.avg
        print("top5_avg:", top5_avg)

        batch_timee = record_list['batch_time']
        batch_timee_avg = batch_timee.avg
        print("batch_timee_avg:", batch_timee_avg)

        reader_timee = record_list['reader_time']
        reader_timee_avg = reader_timee.avg
        print("reader_timee_avg:", reader_timee_avg)


        writer.add_scalar(tag="loss", step=epoch + 1, value=loss_avg)
        # writer.add_scalar(tag="lr", step=epoch + 1, value=record_list[0].get("lr"))
        writer.add_scalar(tag="lr", step=epoch + 1, value=lr_avg)
        writer.add_scalar(tag="top1", step=epoch + 1, value=top1_avg)
        writer.add_scalar(tag="top5", step=epoch + 1, value=top5_avg)
        writer.add_scalar(tag="batch_time", step=epoch + 1, value=batch_timee_avg)
        writer.add_scalar(tag="reader_time", step=epoch + 1, value=reader_timee_avg)
